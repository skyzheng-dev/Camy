...
export default function Post({post} : PostProps) {
  ...
  const {user} = useUser()
  //user stored in the convex database
  ...
  return (
    <View style={styles.post}>
      ...
      <View style={styles.postHeader}>
        <Link href={
          currentUser?._id === post.author._id ? "/(tabs)/profile" : `/user/${post.author._id}`} asChild
        >
            <TouchableOpacity style={styles.postHeaderLeft}>
                <Image source={post.author.image} style={styles.postAvatar} contentFit="cover" transition={200} cachePolicy="memory-disk"></Image>
        ...
        {post.author._id === currentUser?._id ? (
          ...
          <TouchableOpacity>
            <Ionicons name='ellipsis-horizontal' size={20} color={COLORS.white}></Ionicons>
          </TouchableOpacity>
        )}
      ...
      {/* posted image */}
      <Image source={post.imageUrl} style={styles.postImage} contentFit="cover" transition={200} cachePolicy="memory-disk"></Image>

      {/* post actions */}
      <View style={styles.postActions}>
        <View style= {styles.postActionsLeft}>
          <TouchableOpacity>
            <Ionicons onPress={handleLike} name={isLiked? "heart" : "heart-outline"} size={24} color={isLiked? COLORS.primary : COLORS.white}></Ionicons>
          </TouchableOpacity>
